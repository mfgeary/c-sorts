...
quickSortR(int array[], int start, int stop)
{
  ...
  int compars = 0;
  //index of end of section of array with values less than pivot
  int i = start;
  ...
  if(start >= stop - 1) {
    ...
    //recursive case
    //index stops before it reaches pivot
    while(j < stop - 1) {
      //divide array into subarray with elements less than pivot
      if(array[j] <= pivot) {
	temp = array[i];
	array[i] = array[j];
	array[j] = temp;
	...
	j++;
	i++;
	//subarray with elements great than pivot
      } else {
	j++;
	compars++;
      }
    ...
    //move pivot to appropriate section of array
    temp = array[i];
    array[i] = array[j];
    array[j] = temp;

    //recursion on section of array <= pivot
